...
theorem spend_unsound :
    ¬ spend_sound_theorem orderMinimalSingleValidator datum
...
theorem spend_complete :
    ∀ (datum : OrderDatum),
    spend_complete_theorem orderMinimalSingleValidator datum
      (encode4 datum) (encode4 datum) (Data.Constr 0 [Data.I 0])
    := by blaster
...
theorem close_sound :
    ∀ (datum : OrderDatum),
    close_sound_theorem orderMinimalSingleValidator datum (encode4 datum)
    := by blaster

/-- Close is complete: the owner can spend. -/
theorem close_complete :
    ∀ (datum : OrderDatum),
    close_complete_theorem orderMinimalSingleValidator datum (encode4 datum)
    := by blaster
...
theorem no_double_satisfaction :
    ...
    no_double_satisfaction_theorem orderMinimalSingleValidator datum
