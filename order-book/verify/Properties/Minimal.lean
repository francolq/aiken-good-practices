...
theorem spend_unsound :
    ¬ spend_sound_theorem mResolve.prop datum (encode4 datum) (encode4 datum)
...
theorem spend_complete :
    ∀ (datum : OrderDatum),
    spend_complete_theorem mResolve.prop datum
...
theorem close_sound :
    ∀ (datum : OrderDatum),
    close_sound_theorem mClose.prop datum (encode4 datum)
    := by blaster

/-- Close is complete: the owner can spend. -/
theorem close_complete :
    ∀ (datum : OrderDatum),
    close_complete_theorem mClose.prop datum (encode4 datum)
    := by blaster
...
theorem double_satisfaction_fails :
    ¬ no_double_satisfaction_theorem mNoDsat.prop datum
        (encode4 datum) (encode4 datum) (Data.Constr 0 [Data.I 0])
    := by blaster
