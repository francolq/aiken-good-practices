#!/usr/bin/awk -f
# Filter for `lake build` output that annotates each
# `<file>.lean:<line>:<col>:` location with the nearest preceding
# `theorem|lemma|example|def <name>` so messages from blaster
# (`✅ Valid`, `❌ Falsified`, etc.) and from `#import_uplc` identify
# the property being checked, not just the file.
#
# Usage:
...

{
  ...
  while (match(rest, /[A-Za-z0-9_./-]+\.lean:[0-9]+:[0-9]+:/)) {
    ...
    lineno = parts[n - 2] + 0
    if (file !~ /^\//) {
      candidate = "verify/" file
      if ((getline _ < candidate) >= 0) { close(candidate); file = candidate }
